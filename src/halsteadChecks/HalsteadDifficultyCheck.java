...
public class HalsteadDifficultyCheck extends AbstractCheck {
	...
	public double getHalsteadDifficulty() {
		...
		Enumeration<Integer> operatorsItr = this.uniqueOperators.keys(); // .keySet().iterator();
		Enumeration<Integer> operandsItr = this.uniqueOperators.keys(); // .keySet().iterator();
		
		// Sets number of unique operators
		while (operatorsItr.hasMoreElements()) {
			uniqueOperators++;
			operatorsItr.nextElement();
		}
		
		// Sets number of unique operands
		while (operandsItr.hasMoreElements()) {
			uniqueOperands++;
			operandsItr.nextElement();
		}
		...
		double difficulty = (((double)uniqueOperators / 2) * (double)operands) / (double)uniqueOperands;
		return difficulty;
	}
